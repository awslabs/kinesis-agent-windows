...
namespace Amazon.KinesisTap.Core
{
    ...
    public class HighCapacityBuffer<T> : Buffer<T>
    {
        ...
        public override void Add(T item)
        {
            ...
            if (Count < _sizeHint)
            {
                _logger.LogTrace("[{0}] Buffer not full, adding batch to buffer with default behavior.", nameof(HighCapacityBuffer<T>.Add));
                base.Add(item);
                return;
            }
            ...
            // items.
            _logger.LogTrace("[{0}] Buffer full, adding item to persistent queue instead.", nameof(HighCapacityBuffer<T>.Add));
            if (_lowPriorityQueue.TryEnqueue(item)) return;

            // If adding to the persistent queue fails, revert back to the base adding behavior (block till there is
            // space in the buffer).
            _logger.LogWarning("[{0}] Failed to enqueue item in lower priority queue. Attempting to requeue in buffer", nameof(HighCapacityBuffer<T>.Add));
        ...
        protected override bool LowPriorityPumpOne(Action<T> onNext)
        {
            ...
            _logger.LogTrace("[{0}] Pumping one batch from low priority queue to sink.", nameof(HighCapacityBuffer<T>.LowPriorityPumpOne));
            onNext(item);
            return true;
        }
    }
