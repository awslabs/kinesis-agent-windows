...
namespace Amazon.KinesisTap.Core
{
    public class Envelope<T> : IEnvelope<T>
    {
        ...
        public Envelope(T data, DateTime timestamp, RecordBookmark bookmarkData, long position)
        {
            _data = data;
            _timeStamp = timestamp;
            Position = position;
        ...
        public RecordBookmark BookmarkData { get; set; }
        ...
        public virtual string GetMessage(string format)
        {
            ...
            if (string.Equals(ConfigConstants.FORMAT_JSON, format, StringComparison.OrdinalIgnoreCase))
                return ToJson();

            if (string.Equals(ConfigConstants.FORMAT_XML, format, StringComparison.OrdinalIgnoreCase))
                return ToXml();
        ...
        public virtual MemoryStream GetMessageStream(string format)
        {
            ...
            WriteMessageToStream(format, ms);
            return ms;
        }
        ...
        public virtual void WriteMessageToStream(string format, MemoryStream memoryStream)
        {
            ...
            if (string.Equals(ConfigConstants.FORMAT_JSON, format, StringComparison.CurrentCultureIgnoreCase))
            {
                ...
                {
                    if (_data is IJsonConvertable convertable)
                        ...
                        SerializationUtility.Json.Serialize(sw, _data);
                }
        ...
        protected virtual string ToJson()
        {
            ...
            // IJsonConvertable is an interface that custom data types implement that indicate
            // that they use a custom method to serialize the object when the target format is JSON.
            if (_data is IJsonConvertable convertable) return convertable.ToJson();
        ...
        protected virtual string ToXml()
        {
            ...
            var serializer = SerializationUtility.XmlSerializers.GetOrAdd(_data.GetType(), (key) => new XmlSerializer(key));
